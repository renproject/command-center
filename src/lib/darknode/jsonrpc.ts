...
import { RPCResponse } from "../../components/hyperdrivePage/hyperdriveContainer";
...
export const queryStat = async (lightnode: string, darknodeID: string) => {
    ...
    const result = (await Axios.post<RPCResponse<ResponseQueryStat>>(`${lightnode}?id=${darknodeID}`, request)).data.result;
