...
const getBlocks = async (network: RenNetworkDetails): Promise<List<Block>> => {
    ...
    do {
        ...
        (response.blocks === null || response.blocks.length === 0) &&
        i < 5
    );
    return response.blocks
        ? List(response.blocks).sort(
...
const useRenVMContainer = () => {
    ...
    const getBlock = async (blockNumber: number) => {
        ...
        if (blocks) {
            ...
            if (
                first &&
                last &&
                first.header.height >= blockNumber &&
                last.header.height <= blockNumber
            ) {
        ...
        if (!newCurrentBlock) {
            const request = {
                ...
                params: { n: 5, blockHeight: blockNumber },
                id: 67,
            };
            const response = (
                await retryNTimes(
    ...
    const getTransaction = async (
        ...
        if (
            ...
            !options.skipCache &&
            transactions &&
            transactions.get(txHashHex)
        ) {
            currentTransaction = transactions.get(txHashHex);
        } else {
            if (network) {
                try {
                    currentTransaction = await new RenJS(
                        network.name,
                    ).renVM.queryTx(
