...
export const DarknodePage = () => {
    ...
    const { darknodeDetails, darknodeNames, storeDarknodeName, addRegisteringDarknode } = NetworkContainer.useContainer();
    ...
    let readOnly = true;
    try {
        readOnly = !details || !address || details.operator.toLowerCase() !== address.toLowerCase();
    } catch (error) {
        console.error(error);
    }
