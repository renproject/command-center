...
const ColoredBanner: React.FC<{ token: Token }> = ({ token }) => {
  return (
    <div className={`colored-banner colored-banner--${token.toLowerCase()}`} />
  );
};
  ...
  const renderButtons = () => {
    ...
    switch (stage) {
      case Stage.Pending:
        return (
          <div className="popup--buttons">
            <button className="sign--button button--white" onClick={onCancel}>
              Cancel
            </button>
            <button
              className="sign--button button"
              disabled={selectedAddress === null && !newAddressValid}
      ...
      case Stage.WithdrawingToken:
        return (
          <div className="popup--buttons">{/* <Loading alt={true} /> */}</div>
        );
      case Stage.Done:
      case Stage.DoneToken:
        return (
          <div className="popup--buttons">
            <button className="sign--button button" onClick={onDone}>
              Close
            </button>
          </div>
        );
        ...
        <div className="popup--top">
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            {stage === Stage.Pending || stage === Stage.Error ? (
              ...
              >
                Withdraw as ren{token}
              </button>
            ) : null}
          </div>
          <h2>
            {stage === Stage.Withdrawing ? (
              <>Withdrawing {token}</>
            ) : stage === Stage.WithdrawingToken ? (
              <>Withdrawing ren{token}</>
            ) : stage === Stage.Done ? (
              <>{token} withdrawn</>
              ...
              <>
                Enter your {tokenDetails ? tokenDetails.name : token} withdraw
                address
              </>
            )}
          </h2>
          ...
          {stage === Stage.Pending || stage === Stage.Error ? (
            <>
              <div
                className={`withdraw--addresses ${
                  withdrawAddresses.size === 0
                    ? "withdraw--addresses--empty"
                    : ""
                }`}
              >
                {withdrawAddresses
                  .get(token, List<string>())
                  .map((withdrawAddress: string) => {
                    return (
                      <div
                        key={withdrawAddress}
                      ...
                      >
                        ...
                        <button
                          onClick={onClickCopy}
                          data-addr={withdrawAddress}
                          className="withdraw--address--remove"
                        >
                          <FontAwesomeIcon
                            icon={faCopy as FontAwesomeIconProps["icon"]}
                            pull="right"
                          />
                        </button>
                        <button
          ...
          {stage === Stage.Done ? (
            <div className="withdraw--loading">
              <p>
                {token} withdrawn. It will be deposited to your address after 30
        ...
        <div className="popup--bottom">
          {stage === Stage.Pending || stage === Stage.Error ? (
            <form onSubmit={addNewAddress}>
              <div
                className={`new-address--outer ${
                  newAddressValid ? "input--valid" : ""
                }`}
              >
                <input
                  type="text"
                  ...
                  className={`new-address ${
                    selectedAddress ? "new-address--faded" : ""
                  }`}
                  onChange={handleAddressInput}
                />
                <label className="input-label">
                  <span>
                    {tokenDetails ? tokenDetails.name : token} address
                  </span>
                </label>
                {newAddressValid ? (
                  <CheckImage className="new-address--check" />
                ) : null}
              </div>
              <p className="new-address--description">
                This address is where you will receive your rewards.
              </p>
            </form>
          ) : null}
          {renderButtons()}
