...
export const TransactionPreview = ({ tx }: { tx: Tx }) => {
  ...
  if (match) {
    const [token, left, right] = [
      _token.toUpperCase() as Token,
      _left.toUpperCase() as Token,
      _right.toUpperCase() as Token,
    ];
    ...
    return (
      <>
        Shift {amount && amount.value / 10 ** 8} {token} from{" "}
        <TokenIcon white={true} token={left} /> to{" "}
        <TokenIcon white={true} token={right} />
      </>
    );
  }
  return (
...
const txUrl = (
  ...
  switch (token) {
    ...
    case Asset.BTC:
      return `https://chain.so/${isTx ? "tx" : "address"}/BTC${
        network.networkID !== 1 ? "TEST" : ""
      }/${strip0x(txHash)}`;
    case Asset.ZEC:
      return `https://chain.so/${isTx ? "tx" : "address"}/ZEC${
        network.networkID !== 1 ? "TEST" : ""
      }/${strip0x(txHash)}`;
    case Asset.BCH:
      return `https://explorer.bitcoin.com/${
        network.networkID !== 1 ? "t" : ""
      }bch/${isTx ? "tx" : "address"}/${strip0x(txHash)}`;
  }
  return "";
};
...
const RenVMArgValue = ({
  ...
  if (arg.name === "utxo") {
    ...
    if (match) {
      ...
      return (
        <>
          {"{"}"amount": "{utxo.amount}", "txHash": "
          <ExternalLink href={txUrl(txHashHex, token, network)}>
            {utxo.txHash}
          </ExternalLink>
          ", "vOut": "0" {"}"}
        </>
...
export const RenVMTransaction: React.FC<Props> = ({
  network,
  txHash,
  transaction,
  onClose,
}) => {
  const txHash64 = txHash
    ? new EncodedData(txHash, EncodedData.Encodings.HEX).toBase64()
    : undefined;
  ...
  return (
    <TitledSection
      ...
      top={
        <>
          <h1>
            Transaction <span className="monospace">{txHash64}</span>
          </h1>
        </>
      }
    >
        ...
        <>
          <div className="block--txs">
            <div className="block--tx">
              <TransactionPreview tx={transaction.tx} />
            </div>
            <div className="block--tx">
              Status:{" "}
              <span
            ...
            <div className="block--tx">
              Contract:{" "}
              <span className="block--tx--span">{transaction.tx.to}</span>
            </div>
          </div>
          <br />
          <RenVMArgsTable
            network={network}
            renContract={transaction.tx.to}
            title="Inputs"
            args={transaction.tx.in}
          />
          <br />
          {transaction.tx.autogen && transaction.tx.autogen.length > 0 ? (
            <RenVMArgsTable
              network={network}
              renContract={transaction.tx.to}
              title="Generated values"
              args={transaction.tx.autogen}
            />
          ) : null}
          <br />
          {transaction.tx.out && transaction.tx.out.length > 0 ? (
            <RenVMArgsTable
              network={network}
              renContract={transaction.tx.to}
              title="Outputs"
              args={transaction.tx.out}
            />
          ) : null}
          <br />
