...
export const RenVMStatsPage = () => {
    ...
    useEffect(() => {
        if (txHash) { getTransaction(txHash).catch(console.error); }
        const interval = setInterval(() => {
            if (txHash) { getTransaction(txHash, { skipCache: true }).catch(console.error); }
        }, 1000 * 10);
        return () => clearInterval(interval);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [txHash]);
    ...
    return (
        ...
        >
            ...
            {txHash ?
                <RenVMTransaction network={network} txHash={txHash} transaction={currentTransactionHash === txHash ? currentTransaction : undefined} onClose={onClose} /> :
                null
            }
            {blockNumber || blockNumber === 0 ?
                <RenVMBlock blockNumber={blockNumber} block={currentBlockNumber === blockNumber ? currentBlock : undefined} onClose={onClose} onTxClick={onTxClick} onBlockClick={onClick} /> :
                null
            }
