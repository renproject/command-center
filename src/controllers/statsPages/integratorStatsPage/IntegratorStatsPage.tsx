...
export const IntegratorStatsPage = () => {
  ...
  const {
    ...
    activeIntegrator,
    setActiveIntegrator,
  } = IntegratorsContainer.useContainer();
  ...
  for (
    let i = 0;
    i <
    (!Array.isArray(filteredPage) || filteredPage.length === 0
      ? ROWS_PER_PAGE - 1
      : ROWS_PER_PAGE - filteredPage.length);
    i++
  ) {
    empty.push(i);
  }
    ...
    <div className="integrators-page container">
      <table className="integrators-top">
        <thead>
          <tr>
            ...
            <th className="col-2" />
            <th className="col-3">
              24 Hr <span className="integrators-large">Volume</span>
            </th>
            <th className="col-4">
              All Time <span className="integrators-large">Volume</span>
            </th>
          </tr>
        </thead>
        <tbody>
          {!filteredPage ? (
          ...
          ) : typeof filteredPage === "string" ? (
            <EmptyRow>Error loading integrators: {currentPage}</EmptyRow>
          ) : filteredPage.length === 0 ? (
            <EmptyRow>
              No results.{" "}
              <button className="text-button" onClick={goToPreviousPage}>
                Previous page
              </button>
            </EmptyRow>
          ) : (
            filteredPage.map((integrator, i) => (
              <IntegratorRow
                key={integrator.now.id}
                index={i + 1}
                integrator={integrator}
                isActive={activeIntegrator === integrator.now.id}
                setActiveIntegrator={setActiveIntegrator}
              />
            ))
          )}
          {empty.map((i) => {
      ...
      <div className="integrators-bottom">
        ...
        <div className="integrators-bottom-right">
          <button disabled={page === 0} onClick={goToFirstPage}>
            <RemoteStart />
          </button>
          ...
          <button
            disabled={
              !Array.isArray(currentPage) || currentPage.length < ROWS_PER_PAGE
            }
