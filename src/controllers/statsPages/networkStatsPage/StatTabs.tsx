...
const Tab: React.FC<{ value: StatTab, selected: string, onChange: (value: StatTab) => void }> = ({ value, selected, onChange, children }) => {
...
export const StatTabs = ({ selected, onChange, volumePeriod, assetsPeriod }: { selected: string, onChange: (value: StatTab) => void, volumePeriod: PeriodType, assetsPeriod: PeriodType | null }) => {
    return (
        <div className="stat-tabs">
            ...
            <Tab value={StatTab.DigitalAssets} selected={selected} onChange={onChange}>Digital Assets {assetsPeriod ? <>({assetsPeriod === PeriodType.ALL ? "ALL" : <>1{assetsPeriod.slice(0, 1).toUpperCase()}</>})</> : null}</Tab>
        </div>
    );
};
