...
export class ErrorBoundary extends React.Component<Props, typeof defaultState> {
     ...
     */
    public render() {
        const { children, popup, onCancel, className, defaultValue, ...props } = this.props;
        ...
        if (this.state.errorInfo) {
            ...
            return (
                <div
                    ...
                    {...props}
                    className={classNames(this.props.popup ? "popup" : "error-boundary--standard", "error-boundary", className)}
                >
                    <div className="error-boundary--header">
                        <h2>Something went wrong.</h2>
                        {!popup ? <button className="error-boundary--feedback" onClick={showReportDialog}>Report feedback</button> : null}
                    </div>
                    ...
                    {popup ? <div className="popup--buttons">
                        ...
                        <button className="error-boundary--feedback" onClick={showReportDialog}>Report feedback</button>
                    </div> : null
                    }
                </div>
            );
