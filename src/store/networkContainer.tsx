...
const useNetworkContainer = () => {
      ...
      FeeTokens.map(async (_tokenDetails, token) => {
        try {
          ...
          if (renVM && token === Token.BTC)
            return renVM.currentEpoch.rewardShareBTC.decimalPlaces(0);
          if (renVM && token === Token.ZEC)
            return renVM.currentEpoch.rewardShareZEC.decimalPlaces(0);
          if (renVM && token === Token.BCH)
            return renVM.currentEpoch.rewardShareBCH.decimalPlaces(0);
  ...
  const showRegisterPopup = async (
    ...
    const steps = [
      {
        call: step1,
        name: `Approve ${renNetwork.name === "chaosnet" ? "10K" : "100K"} REN`,
      },
